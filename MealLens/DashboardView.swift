    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section {
                    Text("사진은 기기 내에서 분석하고 식사 기록은 이 기기에 저장해요. 음식 종류·조리법·중량에 따라 실제 영양값은 달라집니다. 기본 식품값은 추정값이며, 의료 또는 식이 처방용이 아닙니다.")
        ...
        List {
            ...
            Section("확인한 음식") {
                ForEach(meal.items) { item in
                    LabeledContent(FoodLabelFormatter.storedName(item.name), value: "\(item.grams.formatted()) g · \(Int(item.nutrients.calories)) kcal")
                }
            }
